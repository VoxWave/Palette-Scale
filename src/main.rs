...
fn main() {
    ...
    new_image.save(format!("{}_resized.png", path.file_stem().unwrap().to_string_lossy())).unwrap();
}
...
fn palette_scale(image: &RgbaImage, scale: u32) -> RgbaImage {
    ...
    let mut resized = resize(image, image.width()*scale, image.height()*scale, Lanczos3);
    for x in 0..resized.width() {
        for y in 0..resized.height() {
            ...
            let mut color_distances = palette.iter().map(|color| {
                (color, dist_squared(color, &pixel_color))
            }).collect::<Vec<_>>();
            ...
            let index = match color_distances.iter().enumerate().find(|(_, (_, dist))| *dist != first_val) {
                Some((index, _)) => index,
                None => 1,
            };
...
fn dist_squared(a: &Rgba<u8>, b: &Rgba<u8>) -> i32 {
    (a.0[0] as i32 - b.0[0] as i32).pow(2) + (a.0[1] as i32 - b.0[1] as i32).pow(2) + (a.0[2] as i32 - b.0[2] as i32).pow(2) + (a.0[3] as i32 - b.0[3] as i32).pow(2) 
}